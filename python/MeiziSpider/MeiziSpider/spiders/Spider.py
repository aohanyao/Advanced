...
class MeiziSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=('http://www.mzitu.com/\d{1,6}',),
                           deny='http://www.mzitu.com/\d{1,6}/\d{1,6}'), callback='parse_item',
             follow=True),
    )
    headers = {
    ...
    def parse_item(self, response):
        ...
        max_num = response.xpath(
            "descendant::div[@class='main']/div[@class='content']/div[@class='pagenavi']/a[last("
            ")-1]/span/text()").extract_first(
            default="N/A")
        item['name'] = response.xpath("./*//div[@class='main']/div[1]/h2/text()").extract_first(
            default="N/A")
    ...
    def img_url(self, response):
        ...
        img_urls = response.xpath(
            "descendant::div[@class='main-image']/descendant::img/@src").extract()
